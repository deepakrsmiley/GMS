...
function ShiftReport() {
  ...
  const load = async () => {
    ...
    setData(null);
    try {
      const url = `${API}/report/shift?date=${date}`;
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    ...
                    <tbody>
                      {s.bills.map((b, i) => (
                        <tr key={i} style={{ borderTop: "1px solid #f1f5f9", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                          ...
                          <td style={{ ...tdS, fontWeight: 700, color: "#1d4ed8", fontFamily: "ui-monospace, monospace" }}>{b.patientId || "—"}</td>
                    ...
                    <tfoot>
                      <tr style={{ background: "#f1f5f9", fontWeight: 700 }}>
                        <td colSpan={5} style={tdS}>Shift Total</td>
