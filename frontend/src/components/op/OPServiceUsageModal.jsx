...
// Logs equipment/procedure usage (ECG, Nebulizer, dressing, injection, etc.) against
// an OP visit. Each saved entry is automatically picked up as a billable line item
// next time an OP bill is generated for this patient (see billingService.js) - no
// manual bill entry required. Uses the same rate list (ServiceMaster) as the IP side.
export default function OPServiceUsageModal({ registration, isOpen, onClose }) {
  ...
  const addMutation = useMutation({
    mutationFn: (payload) => api.post(`/op/${registration._id}/service-usage`, payload),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (usageId) => api.delete(`/op/${registration._id}/service-usage/${usageId}`),
    onSuccess: () => {
  ...
  return (
    ...
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="mb-5 pb-5 border-b border-gray-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input
              className={field}
              placeholder="e.g. Done in OPD room 2"
              value={form.notes}
      ...
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-sm font-semibold text-gray-700">Logged this visit</h4>
        {usages.length > 0 && (
          <span className="text-sm text-gray-500">
